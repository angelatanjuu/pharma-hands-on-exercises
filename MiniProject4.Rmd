...
# Age Count
Agegrp_N_cnt <- adsl_eff %>%
  group_by(TRT01AN, TRT01A, AGEGR1, AGEGR1N) %>%
  count(name = "age_total")

age_n_cnt <- adsl_eff %>%
  group_by(TRT01AN, TRT01A, SEX, AGEGR1, AGEGR1N) %>%
  count(name = "age_n")

age_mrg_cnt <- age_n_cnt %>% 
  left_join(Agegrp_N_cnt, 
            by = c("TRT01AN", "TRT01A", "AGEGR1", "AGEGR1N"))
...
age_n_pct <- age_pct %>%
  mutate(npct = paste(age_n, paste0("(", perc_achar, ")"))) %>% 
  select(AGEGR1, AGEGR1N, TRT01A, SEX, npct)
...
```{r}
# Bind Age rows
dm_agecomb <- bind_rows(agestat_cat[1,],age_cat, agestat_cat[-1,])
dm_agecomb

# Bind Ethnicity rows
dm_ethniccomb <- rbind("",ethnic_cat)
dm_ethniccomb[1,1] <- "Ethnicity, n(%)"
dm_ethniccomb

# Bind Race rows
dm_racecomb <- rbind("",race_cat)
dm_racecomb[1,1] <- "Race, n(%)"
dm_racecomb

# Bind all rows
dm_allcomb <- bind_rows(dm_agecomb, dm_ethniccomb, dm_racecomb)
dm_allcomb
```