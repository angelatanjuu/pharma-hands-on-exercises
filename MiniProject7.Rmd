...
plot_adlb <- function(inputfileADLB, xlabel, ylabel){
ALT <- import(inputfileADLB) %>%
  filter(PARAMCD == "ALT")
...
#Red points
HighLow <- ALT2 %>%
  filter(LBNRIND %in% c("HIGH", "LOW") & WEEK %in% c(2, 4, 6, 8, 16, 24, 26))

challenge <- ALT2 %>% 
  filter(WEEK %in% c(2, 4, 6, 8, 16, 24, 26)) %>% 
  ggplot(mapping = aes(x = LBSTRESN, y = as.factor(WEEK))) +
  coord_cartesian(xlim=c(0, 150)) +
  stat_dotsinterval(slab_shape = 19, slab_alpha = 0) +
  geom_point(data = HighLow, color = "red") + 
...
adlb_file <- "https://github.com/phuse-org/phuse-scripts/raw/master/data/adam/cdisc/adlbc.xpt"

plot_adlb(adlb_file, "Alanine Aminotransferase U/L", "Week")
```
