...
age_stat<- adsl_eff %>%
  group_by(TRT01AN,TRT01A,SEX) %>%
  summarize(mean = (mean(AGE) %>% round(digits = 0) %>% format(nsmall=0))  ,
...
```{r}
weight_stat<- na.omit(adsl_eff) %>%
  group_by(TRT01AN,TRT01A,SEX) %>%
  summarize(mean = (mean(WEIGHTBL) %>% round(digits = 0) %>% format(nsmall=0))  ,
...
height_stat<- na.omit(adsl_eff) %>%
  group_by(TRT01AN,TRT01A,SEX) %>%
  summarize(mean = (mean(HEIGHTBL) %>% round(digits = 0) %>% format(nsmall=0))  ,
